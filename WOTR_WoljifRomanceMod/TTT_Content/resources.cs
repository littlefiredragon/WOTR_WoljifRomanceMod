...
namespace TabletopTweaks
{
    static class Resources
    {
        ...
        public static T GetBlueprint<T>(BlueprintGuid id) where T : SimpleBlueprint
        {
            ...
            T value = asset as T;
            if (value == null) { WOTR_WoljifRomanceMod.Main.Log($"COULD NOT LOAD: {id} - {typeof(T)}"); }
            return value;
        }
        ...
        public static void AddBlueprint([NotNull] SimpleBlueprint blueprint, BlueprintGuid assetId)
        {
            ...
            if (loadedBlueprint == null)
            {
                ...
                blueprint.OnEnable();
                WOTR_WoljifRomanceMod.Main.LogPatch("Added", blueprint);
            }
            else
            {
                WOTR_WoljifRomanceMod.Main.Log($"Failed to Add: {blueprint.name}");
                WOTR_WoljifRomanceMod.Main.Log($"Asset ID: {assetId} already in use by: {loadedBlueprint.name}");
            }
        }
    }
}