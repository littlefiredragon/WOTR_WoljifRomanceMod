        ...
        public BlueprintGuid GetGUID(string name)
        {
            ...
            if (!NewBlueprints.TryGetValue(name, out Id))
            {
                ...
                if (!AutoGenerated.TryGetValue(name, out Id))
                {
                    ...
                    WOTR_WoljifRomanceMod.Main.LogDebug($"Generated new GUID: {name} - {Id}");
                }
                else
                {
                    WOTR_WoljifRomanceMod.Main.LogDebug($"WARNING: GUID: {name} - {Id} is autogenerated");
                }
#endif
            }
            if (Id == null) { WOTR_WoljifRomanceMod.Main.Log($"ERROR: GUID for {name} not found"); }
            UsedGUIDs[name] = Id;
